...
/**
 * @author jbriggs
 * Implements a double auction order book.
 * The market moves when market orders fulfill limit orders.
 *   - limit orders don't ever execute when they're placed
 * 
 * Internally, prices are stored as integers to avoid the 
 * imprecision of doubles causing faults
 * 
...
public class DoubleAuctionOrderBook implements OrderBook
{
	...
	public DoubleAuctionOrderBook(ContModel myWorld) 
	{
...
//		this.nextTransactionID=0;
	...
	// these will return a transactionID. the price must be rounded to thousandths of a unit. quantity is a whole number to purchase (initially this will probably be 1), 
	// expirationSteps is the number of ticks after which the order will be considered to have been cancelled
	// NOTE: prices should be rounded to the nearest 1/granularity or there will be discrepancies
	public synchronized boolean PlaceLimitOrder(LimitOrder order) 
	{
		...
		// This means it's possible to achieve a negative spread, if an offer is made to 
		// sell at below the bid price or buy at above the ask price. This will persist 
		// until a market order occurs. In practice, this should almost never occur.
		...
		{ // purchase order
			...
			if (internalPrice>bidPrice || !buyOrderQueues.containsKey(bidPrice)) 
            { // if this purchase is bidding more than the rest, or there are no others, set it
	...
	public synchronized double ExecuteMarketOrder(OrderType type, int quantity) throws LiquidityException
	{
		...
		if (type==OrderType.PURCHASE) 
		{
			...
			for (int salePrice = askPrice; salePrice <= highestAsk; salePrice++)
			{
				...
				if (orders != null) {
                    ...
                    askPrice=salePrice; // update the askingPrice to reflect this order
                    if (quantity==0) {
                    	return totalPrice;
                    }
					...
					while (!orders.isEmpty())
					{
						...
						if (o.quantityPending()>quantity) 
						{
						    ...
						    orders.add(0, o); // adds the order back in since this won't exhaust it
			...
			if (quantity!=0) {
				throw new LiquidityException(origQuant-quantity, totalPrice); // TODO: add params
			}
		} 
		else // SALE 
		{ 
			...
			for (int salePrice = bidPrice; salePrice >= lowestBid; salePrice--)
			{
				...
				if (orders != null) {
                    ...
                    bidPrice=salePrice; // update the bidPrice to reflect this order
                    if (quantity==0) {
                    	return totalPrice;
                    }
					...
					while (!orders.isEmpty())
					{
						...
						{
						    curQuantity=quantity;
						    orders.add(0, o); // adds the order back in queue since this won't fully execute it
						} else if (o.quantityPending()==quantity) {
							curQuantity=quantity;
						} else {
							curQuantity = quantity-o.quantityPending(); 
		...
		// only get here if the queue is empty and we transacted exactly what we wanted.
		return totalPrice; 
	}
...
/*	public synchronized boolean registerGUISeries(XYSeries BuyOrdersSeries, 
	...
	{
		buyOrdersSeries=BuyOrdersSeries;
		sellOrdersSeries=SellOrdersSeries;
		return true;
	}
*/	
